...
if not st.session_state.setup_complete:
    ...
    st.session_state["experience"] = st.text_area(label="Experience", max_chars=200, height=None, value=st.session_state["experience"], placeholder="Describe your experience")
...
if st.session_state.setup_complete and not st.session_state.feedback_shown and not st.session_state.chat_complete:
    ...
    client = OpenAI(api_key=st.secrets["OPENAI_API_KEY"], base_url="https://models.github.ai/inference")

    if "openai_model" not in st.session_state:
        st.session_state["openai_model"] = "openai/gpt-4.1"

    if not st.session_state.messages:
        st.session_state["messages"] = [{
            "role": "system",
             "content" : (f"You are an HR executive that interviews an interviewee called {st.session_state["name"]} with experience {st.session_state["experience"]} and skills{st.session_state["skills"]}. You should interview them for the positin {st.session_state["level"]} {st.session_state["position"]} at the company {st.session_state["company"]}.")}]
...
if st.session_state.feedback_shown:
    ...
    feedback_client = OpenAI(api_key=st.secrets["OPENAI_API_KEY"], base_url="https://models.github.ai/inference")
